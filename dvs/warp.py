...
def warp_video(mesh_path, video_path, save_path, losses = None):
    if type(mesh_path) == str:
        mesh_data = load_gyro_mesh(mesh_path)
        grid_data = mesh_data["warping grid"]
        # mesh_data = load_mesh(mesh_path)
    ...
    t1 = time.time()
    # frame_array = rendering(mesh_data[:length], frame_array[:length], size)
    frame_array = warpping_rast(grid_data[:length], frame_array[:length], losses = losses)
    t2 = time.time()
    print(t2-t1)
    save_video(save_path,frame_array, fps, size, losses = losses)
...
def warpping_rast(grid_data, frame_array, losses = None):
    ...
    for i in range(0, min(len(frame_array), grid_data.shape[0])):
        if i % 20 == 0:
            print(i)
        t1 = time.time()
        frame = warpping_one_frame_rast(frame_array.pop(0), grid_data[i])
        t2 = time.time()
        print(t2 - t1, losses[i])
        output.append(frame)
    return output
