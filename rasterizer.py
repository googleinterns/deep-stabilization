...
def Rasterization(image, grid):
    ...
    xratio = xlength / height
    yratio = ylength / width
    ...
    tran_triangle[:,:,:,0] = (wapper_triangle[:,:,:,0] - xmin.view(-1,1,1).to(device)/height) / xratio
    tran_triangle[:,:,:,1] = (wapper_triangle[:,:,:,1] - ymin.view(-1,1,1).to(device)/width) / yratio

    mask = triangle2mask(tran_triangle, xlength, ylength) # consuming
    mask = torch.unsqueeze(mask, 4)
    origin_triangle = torch.unsqueeze(origin_triangle, 1)

    grid_sample = origin_triangle * mask # consuming
    grid_sample = torch.sum(torch.sum(grid_sample, axis = 3), axis = 2).view(-1,xlength,ylength,2) # consuming
    ...
    grid_merge = torch.zeros((gxmax-gxmin,gymax-gymin,2)).to(device)
    for i in range(grid_sample.size()[0]):
        ...
        grid_merge[x_s:x_e, y_s:y_e, :] += grid_sample[i, :, :, :]

    # crop_xmin = min(0, gxmin)
    # crop_ymin = min(0, gymin)
    grid_merge = grid_merge[-gxmin:-gxmin+height, -gymin:-gymin+width, :] 
    
    shift = torch.tensor([0.5/height,0.5/width])[None, None, :].to(device)
    grid_merge = (grid_merge + 1*shift) * 2 - 1
    grid_merge = torch.stack((grid_merge[:,:,1],grid_merge[:,:,0]),2) # Unknown why need flip
...
def grid_size(upper_triangle, lower_triangle, height, width):
    ...
    xmax = torch.floor(torch.max(wapper_grid[:,:,0]*height, 1)[0]) + 1
    ymax = torch.floor(torch.max(wapper_grid[:,:,1]*width, 1)[0]) + 1
    xmin = torch.floor(torch.min(wapper_grid[:,:,0]*height, 1)[0])
    ymin = torch.floor(torch.min(wapper_grid[:,:,1]*width, 1)[0])
...
def triangle2mask(d, height, width): # d: [N x T x 3 x 2]
    ...
    xs = np.linspace(0, 1, height, endpoint=False)
    ys = np.linspace(0, 1, width, endpoint=False)
    xmesh, ymesh = np.meshgrid(ys, xs)
